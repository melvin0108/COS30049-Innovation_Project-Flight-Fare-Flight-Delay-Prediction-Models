...
def get_user_input():
    ...
    while True:
        ...
        if user_input["SCHEDULED_DEPARTURE"] != user_input["SCHEDULED_ARRIVAL"]:
            break
        else:
    ...
    user_input["TAXI_OUT"] = 0  # Set to default since it's not available
    user_input["WHEELS_OFF"] = 0  # Set to default since it's not available
    ...
    user_input["WHEELS_ON"] = 0  # Set to default since it's not available
    user_input["TAXI_IN"] = 0  # Set to default since it's not available
...
def preprocess_input(user_input):
    ...
    # Define categorical_cols and numerical_cols (without 'ARRIVAL_DELAY' as it is the target)
    ...
    categorical_data = pd.DataFrame(encoder.fit_transform(user_data[categorical_cols]))
    
    # Apply StandardScaler to numerical columns
    scaler = StandardScaler()
    numerical_data = pd.DataFrame(scaler.fit_transform(user_data[numerical_cols]), columns=numerical_cols)
